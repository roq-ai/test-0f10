...
import { getActivities } from 'apiSdk/activities';
...
function DashboardPage() {
  ...
  const activityStat = useSWR<PaginatedInterface<ActivityInterface>>(
    () => `/activities`,
    () => getActivities({ limit: 1, offset: 0 }),
  );
  ...
  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    ...
    {
      title: 'Activities',
      result: activityStat?.data?.totalCount,
      isLoading: activityStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
      title: 'Customers',
